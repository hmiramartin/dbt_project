...
select * from columns as questions
    where exists(select answers.parent_id from {{ ref('stg_stackoverflow_answers') }} as answers
    where questions._pk = answers.parent_id)
    AND creation_date_datetime_utc BETWEEN "{{ var('date_from') }}" AND "{{ var('date_to') }}"